...
pub trait Codec: Sized {
    ...
    fn encode_bytes(self) -> CodecResult<Bytes> {
...
fn encode_headers(headers: HashMap<String, String>, dst: &mut BytesMut) -> CodecResult<()> {
    encode_header_fields::<u16>(headers, dst, &BytesMut::put_u16, &encode_string)
}

fn encode_small_headers(headers: HashMap<String, String>, dst: &mut BytesMut) -> CodecResult<()> {
...
fn decode_headers(src: &mut Bytes) -> CodecResult<HashMap<String, String>> {
    decode_headers_field(src, &Bytes::get_u16, &decode_string)
}

fn decode_small_headers(src: &mut Bytes) -> CodecResult<HashMap<String, String>> {
...
fn decode_checksum(src: &mut Bytes) -> CodecResult<(ChecksumType, Option<u32>)> {
...
fn decode_bitflag<T, F: Fn(u8) -> Option<T>>(byte: u8, decoder: F) -> CodecResult<T> {
    decoder(byte).ok_or_else(|| CodecError::Error(format!("Unknown flag: {}", byte)))
}

fn encode_args(args: VecDeque<Option<Bytes>>, dst: &mut BytesMut) -> CodecResult<()> {
...
fn decode_args(src: &mut Bytes) -> CodecResult<VecDeque<Option<Bytes>>> {
    if src.remaining() == 0 {
...
fn decode_arg(src: &mut Bytes) -> CodecResult<Option<Bytes>> {
    match src.remaining() {
...
fn encode_string<STR: AsRef<str>>(value: STR, dst: &mut BytesMut) -> CodecResult<()> {
    encode_string_field(value, dst, &BytesMut::put_u16)
}

fn encode_small_string<STR: AsRef<str>>(value: STR, dst: &mut BytesMut) -> CodecResult<()> {
...
fn decode_string(src: &mut Bytes) -> CodecResult<String> {
    decode_string_field(src, &Bytes::get_u16)
}
fn decode_small_string(src: &mut Bytes) -> CodecResult<String> {
